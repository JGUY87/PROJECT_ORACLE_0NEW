...
def _resolve_token_chat(chat_id: Optional[str]=None, token: Optional[str]=None) -> tuple[str,str]:
    ...
    if not tok or not cid: raise RuntimeError("TELEGRAM 토큰/채팅ID 없음")
    return str(tok), str(cid)

async def _post_json(url: str, data: Dict[str, Any], timeout: int = 15) -> Dict[str, Any]:
    if aiohttp is None: raise RuntimeError("aiohttp 미설치")
...
async def send_telegram_message(
    ...
    # This function is disabled to prevent network errors.
    # Instead of sending a message, it just returns a success-like dictionary.
    # To re-enable, you would need to fix the network/firewall issue and restore this function.
    print(f"[TELEGRAM DISABLED] Would have sent: {text}")
    return {"ok": True, "result": "message_disabled"}
